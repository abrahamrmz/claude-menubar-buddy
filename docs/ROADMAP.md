...
- **Los tests que trae el refactor de `BuddyCore` no corren en esta máquina**: `swift test` falla con `no such module 'XCTest'` porque sólo hay CommandLineTools, sin Xcode.app. Las mismas propiedades (cobertura especie×mood, nada inalcanzable) se comprobaron con un script aparte.

#### 5.4c ◐ Tres gestos para el koala — arte listo, cableado pendiente (2026-08-19)
`greet`, `yawn` y `dance` — la otra mitad de la 5.4. **63 generaciones** (891 → 828). Sólo el koala; las otras tres degradan.

**Este commit trae sólo el arte, el generador y los docs.** El cableado (disparadores en `MoodEngine`, `MoodPolicy.allMoods`/`gifCandidates`, la burbuja) está escrito, compila y se verificó en vivo, pero se quedó **sin commitear** porque quedó entrelazado con el refactor de `BuddyCore` que estaba en curso en paralelo: `MoodEngine.swift` ya importa ese módulo, y subirlo sin `Sources/BuddyCore/` ni el `Package.swift` que lo declara dejaría el repo sin compilar. Los dos aterrizan juntos. Mientras tanto los tres GIFs son inertes — las cadenas de degradación los cubren, así que no hay estado roto, sólo un gesto que todavía no se dispara.

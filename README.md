...
- **19 pet characters** to choose from (`Settings ▸ Appearance`): a pixel-art panda plus 18 ASCII-art pets reused from the M5Stick Hardware Buddy firmware (cat, axolotl, turtle, dragon, ghost, robot, and more). Each one animates with the choreography its firmware actually plays — the same pose sequence at the same tempo, in the colour that species is drawn in on the device
